...
Regardless, I guess it doesn't really make sense to me to vary both.
Personally, my call would be just to remove $\tau$ altogether from the manuscript and say the simulations use $\Delta t = 1000$---the numerical results should be identical---however I understand that's a touch radical and last minute, so happy to leave it!
If it's common for whatever reason in the literature to include a $\tau$, then so be it.

\quad N.B. Since both varying $\tau$ and varying $\Delta t$ are equivalent, I'd just quickly note here for the future that I reckon the latter would be a little more computationally robust: if I double $\tau$, $\bm u$ should double (making the solver take longer to converge); if I double $\Delta t$, it should not.}
...
\rp This error might be the interpolation error, as we first project the initial condition to a divergence-free space. We will consider this in the future if this error largely affects our numerical simulation. 
